...
const EmotionalCheckIn: React.FC<EmotionalCheckInProps> = ({ onPointsEarned }) => {
  ...
  const [checkInType, setCheckInType] = useState<'parent' | 'child'>('parent');
  ...
  const emotions: Emotion[] = [
    ...
    },
    {
      name: 'Worried',
      emoji: (
        <div className="w-12 h-12 bg-gradient-to-br from-purple-400 to-purple-500 rounded-full shadow-lg flex items-center justify-center transform hover:scale-110 transition-all">
          <span className="text-2xl">😟</span>
        </div>
      ),
    ...
    <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-200 relative">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <h3 className="text-lg font-semibold text-gray-900">Daily Emotional Check-in</h3>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-4">
        {emotions.map((emotion) => (
          <button
            ...
            className={`flex flex-col items-center group relative transition-transform cursor-pointer hover:scale-105 ${
          ...
          >
            ...
            {selectedEmotion === emotion.name && (
              <div className="absolute -inset-2 border-2 border-green-500 rounded-xl opacity-50"></div>
            )}
          </button>
        ))}
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[60]">
          <div className="bg-white rounded-xl p-6 shadow-2xl transform animate-bounce">
            <div className="text-center">
              ...
              <p className="text-gray-600">You earned <span className="font-bold text-green-600">+{isExpired ? 25 : 50}</span> points for your daily check-in.</p>
            </div>
          </div>
        </div>
      )}
